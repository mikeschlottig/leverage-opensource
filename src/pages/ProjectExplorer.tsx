...
import { ArrowRight, BookOpen, BrainCircuit, Code, FileTree, Github, Link as LinkIcon } from 'lucide-react';
...
export default function ProjectExplorer() {
  ...
  if (!project) {
    return (
      <AppLayout>
        <div className="center h-screen text-muted-foreground">Project not found.</div>
      </AppLayout>
    );
  }
  return (
        ...
        <div className="py-8 md:py-10 lg:py-12">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  {isLoadingAnalysis ? (
                    ...
                    <Accordion type="single" collapsible className="w-full">
                      {analysis.mechanisms.map((mech) => (
                        <AccordionItem value={mech.name} key={mech.name}>
                          <AccordionTrigger>{mech.name}</AccordionTrigger>
                          <AccordionContent>
            ...
            <aside className="space-y-6">
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2"><FileTree /> File Tree</CardTitle>
                </CardHeader>
                <CardContent>
