...
export default function ComponentStudio() {
  ...
  const { data: revisions, isLoading: isLoadingRevisions } = useQuery<ComponentSpec[]>({
    ...
    queryFn: () => api(`/api/components/${createdComponent!.id}/revisions`),
    enabled: !!createdComponent,
  });
  useEffect(() => {
  ...
  const handleDownloadZip = async (component: ComponentSpec) => {
    ...
    link.download = `${component.name}-v${component.version}.zip`;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            <motion.main initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="lg:col-span-2">
              <Card className="h-full"><CardHeader><CardTitle className="flex items-center gap-2"><Cuboid /> Live Preview</CardTitle></CardHeader>
                <CardContent>
                  {componentToPreview ? (<ComponentPreview component={componentToPreview} />) : (<div className="w-full h-96 flex items-center justify-center bg-muted/30 rounded-lg border border-dashed"><p className="text-muted-foreground">Your component preview will appear here.</p></div>)}
                </CardContent>
              </Card>
            </motion.main>
                  ...
                  <CardContent className="space-y-2">
                    {revisions && revisions.length > 0 && (
                      <Select onValueChange={(v) => setSelectedVersion(revisions.find(r => r.version === v) || null)}>
                        <SelectTrigger><SelectValue placeholder={`Version: ${componentToPreview.version}`} /></SelectTrigger>
                        <SelectContent>
                          {[createdComponent, ...revisions].map(c => c && <SelectItem key={c.version} value={c.version}>Version {c.version}</SelectItem>)}
                        </SelectContent>
                      </Select>
                    )}
